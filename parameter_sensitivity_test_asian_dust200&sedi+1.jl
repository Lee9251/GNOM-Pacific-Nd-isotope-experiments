...
for β in [:β_EAsia_dust] 
for K in [:K_dust]


    println("\nIncreasing β_EAsia_dust and K_dust by 100%")
    p = Params(; p0..., β => 2 * getfield(p0, β))
    p = Params(; p0..., K => 2 * getfield(p0, K))
...
using DataFrames
using CSV
εNd=rearrange_into_3Darray(εNdmodel, grd)
nlat, nlon, ndepth = size(εNd)
total_points = nlon * nlat * ndepth
# 初始化存储向量
lon_vec = Vector{Float64}(undef, total_points)
lat_vec = Vector{Float64}(undef, total_points)
depth_vec = Vector{Float64}(undef, total_points)
εnd_data = Vector{Float64}(undef, total_points)
# 填充数据
index = 1
for k in 1:ndepth
    for j in 1:nlat
        for i in 1:nlon
            lon_vec[index] = ustrip(grd.lon[i])     # 去除单位
            lat_vec[index] = ustrip(grd.lat[j])     # 去除单位
            depth_vec[index] = ustrip(grd.depth[k]) # 去除单位
            εnd_data[index] = εNd[j, i, k]
            index += 1
        end
    end
end
# 创建 DataFrame
df = DataFrame(Longitude = lon_vec, Latitude = lat_vec, Depth = depth_vec, εNd = εnd_data)
# 导出为 CSV 文件
CSV.write("output_eNd-200%asiandust and -4 sedi.csv", df)
